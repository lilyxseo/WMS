...
import { buildExactLookupQuery, buildNumericLookupQuery, normalizeBarcode, parseVisualizationResponse } from '../functions/api/barcode-lookup.js';
...
test('barcode master query is exact, limited, and safely quotes values', () => {
  ...
  assert.equal(buildNumericLookupQuery('8999999999999', ['B']), 'select A where B = 8999999999999 limit 1');
  assert.equal(buildNumericLookupQuery('0123', ['B']), '', 'leading-zero barcode must remain a string');
});
...
test('protected endpoint performs barcode-first lookup and optional exact SKU fallback', () => {
  ...
  assert.match(endpoint, /buildNumericLookupQuery\(barcode, columns\)/);
  assert.match(endpoint, /buildExactLookupQuery\(barcode, columns\)/);
  assert.match(endpoint, /SKU_PATTERN\.test\(barcode\)/);
  assert.match(endpoint, /findExactInventorySku\(env, barcode\)/);
  assert.match(endpoint, /sku=eq\.\$\{encodeURIComponent\(sku\)\}/);
