...
async function loadWarningPage({preserveScroll=false}={}){const token=++ANOMALY_STATE.lastRenderToken,wrap=document.querySelector('.anomaly-table-wrap'),top=wrap?.scrollTop||0,left=wrap?.scrollLeft||0;setAnomalyLoading(true);try{const headers=await getAuthHeaders(),response=await fetch(`/api/inventory-warnings?${warningQuery()}`,{headers,cache:'no-store'}),payload=await response.json().catch(()=>({}));if(!response.ok||!payload.success||!Array.isArray(payload.rows))throw new Error(payload.message||`HTTP ${response.status}`);if(token!==ANOMALY_STATE.lastRenderToken)return;Object.assign(ANOMALY_STATE,{rows:payload.rows,total:payload.total,summary:payload.summary,categories:payload.categories||{}});updateAnomalyTypeOptions();renderAnomalyRowsOnly();if(preserveScroll&&wrap)requestAnimationFrame(()=>{wrap.scrollTop=top;wrap.scrollLeft=left;});if(payload.partial)toast(`Warning parsial: ${payload.unavailableSources.join(', ')} tidak tersedia`,'warning');}catch(error){if(token===ANOMALY_STATE.lastRenderToken){console.error('[WarningRefresh]',error?.message||error);toast('Gagal memperbarui warning dari server','error');}}finally{if(token===ANOMALY_STATE.lastRenderToken)setAnomalyLoading(false);}}
