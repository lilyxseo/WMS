...
const TRANSACTION_LOCATION_ALIASES={from_location:"from",to_location:"to"};
function getTransactionPresentationCells(row){const cells=Object.fromEntries(Object.entries(row||{}).filter(([k])=>!k.startsWith("_")&&k!=="rowNumber"));for(const [alias,canonical] of Object.entries(TRANSACTION_LOCATION_ALIASES)){if(Object.hasOwn(cells,alias)&&Object.hasOwn(cells,canonical)&&String(cells[alias]??"")===String(cells[canonical]??""))delete cells[alias];}return cells;}
...
function ensureMovementFilterColumns(columns){FILTERABLE_COLUMNS.length=0;columns.forEach(c=>{if(!FILTER_LABELS[c])FILTER_LABELS[c]=String(c).toUpperCase();});}
...
const filterHtml=`<div class='mv-toolbar'><button class='btn-ghost' data-mv-action='reset' data-mv-mode='${mode}'>Reset Filter</button><button class='btn-ghost' data-mv-action='export' data-mv-mode='${mode}'>Export CSV</button><select id='mv-sort-${mode}' data-mv-filter data-mv-mode='${mode}'><option value='latest'>Terbaru</option><option value='oldest'>Terlama</option><option value='sku'>SKU A-Z</option><option value='name'>Nama A-Z</option><option value='qtyDesc'>Qty terbesar</option><option value='qtyAsc'>Qty terkecil</option></select><select id='mv-size-${mode}' data-mv-filter data-mv-mode='${mode}'><option value='25' ${st.pageSize===25?'selected':''}>25</option><option value='50' ${st.pageSize===50?'selected':''}>50</option></select></div>`;
...
function locationParams(page=LOCATION_STATE.page){return {page:String(page),limit:String(LOCATION_STATE.pageSize),search:String(locSearchInput?.value||'').trim(),skuSearch:String(locSkuSearchInput?.value||'').trim(),status:locStatusFilter?.value||'all',type:locTypeFilter?.value||'all',sort:locSort?.value||'skuDesc'};}
...
async function loadLocationPage(page,{prefetch=false}={}){const params=locationParams(page),key=locationCacheKey(params),cached=LOCATION_STATE.pageCache.get(key);if(cached){if(!prefetch){LOCATION_STATE.page=page;LOCATION_STATE.rows=cached.rows;LOCATION_STATE.total=cached.total;LOCATION_STATE.loading=false;drawLocations();prefetchLocationPage(page+1);}return cached;}const token=prefetch?0:++LOCATION_STATE.requestToken;if(!prefetch){LOCATION_STATE.loading=true;LOCATION_STATE.error='';drawLocations();}const data=await fetchLocationJson(`/api/locations?${new URLSearchParams(params)}`);setLimitedCache(LOCATION_STATE.pageCache,key,data);if(prefetch)return data;if(token!==LOCATION_STATE.requestToken)return data;LOCATION_STATE.page=data.page;LOCATION_STATE.rows=data.rows||[];LOCATION_STATE.total=Number(data.total)||0;LOCATION_STATE.loading=false;drawLocations();prefetchLocationPage(page+1);return data;}
function prefetchLocationPage(page){if(page<1||page>Math.ceil(LOCATION_STATE.total/LOCATION_STATE.pageSize))return;const run=()=>loadLocationPage(page,{prefetch:true}).catch(()=>{});if('requestIdleCallback' in window)requestIdleCallback(run,{timeout:1500});else setTimeout(run,0);}
