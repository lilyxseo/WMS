...
if(page==='barang-masuk')return loadTransactionTablePage('in',{page:1});
if(page==='barang-keluar')return loadTransactionTablePage('out',{page:1});
...
if(activePage==='barang-masuk'||activePage==='barang-keluar')return loadTransactionTablePage(activePage==='barang-masuk'?'in':'out',{page:TABLE_STATE[activePage==='barang-masuk'?'in':'out'].page});
...
async function loadTransactionTablePage(mode,{page,limit,search}={}){
const st=TABLE_STATE[mode],requestId=++st.requestId;st.abortController?.abort();const controller=new AbortController();st.abortController=controller;st.loading=true;st.error="";
...
renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);
...
window.APP_STATE=window.APP_STATE||{};if(mode==='in'){window.APP_STATE.barangMasuk=rows;DATA['Barang Masuk']=rows;}else{window.APP_STATE.barangKeluar=rows;DATA['Barang Keluar']=rows;}return rows;
