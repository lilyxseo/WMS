...
function isKeyboardTypingTarget(target){return target instanceof Element&&!!target.closest('input, textarea, select, [contenteditable]:not([contenteditable="false"])');}
function focusBalikanSearch(){const input=document.querySelector('#balikanSearchInput')||balikanSearchInput;if(!input)return;input.focus();input.select();}
async function autoCheckSingleBalikanResult(){
  if(BALIKAN_STATE.shortcutCheckPending)return;
  const {rows}=getBalikanVisibleRows();
  if(rows.length!==1||getPermissions().canUpdate===false){if(rows.length===0)toast('Data tidak ditemukan','error');return;}
  const row=rows[0];
  if(isCheckedValue(row.checked)){showToast('Sudah dipilih','success');resetBalikanSearch();focusBalikanSearch();return;}
  BALIKAN_STATE.shortcutCheckPending=true;
  try{
    await window.toggleBalikanCheck(getBalikanActiveSheetName(row),Number(row.rowNumber),true);
    resetBalikanSearch();
    focusBalikanSearch();
  }catch(_err){}finally{BALIKAN_STATE.shortcutCheckPending=false;}
}
function handleBalikanSearchKeydown(event){
  if(event.key==='Enter'){event.preventDefault();void autoCheckSingleBalikanResult();return;}
  if(event.key==='Escape'){event.preventDefault();resetBalikanSearch();focusBalikanSearch();}
}
function handleBalikanShortcutKeydown(event){
  if(event.ctrlKey&&!event.altKey&&!event.shiftKey&&String(event.key).toLowerCase()==='k'){event.preventDefault();focusBalikanSearch();return;}
  if(isKeyboardTypingTarget(event.target))return;
  if(event.key==='/'&&!event.ctrlKey&&!event.metaKey&&!event.altKey){event.preventDefault();focusBalikanSearch();return;}
  if(event.key==='Escape'){event.preventDefault();resetBalikanSearch();focusBalikanSearch();return;}
  if(event.key===' '){
    const row=event.target instanceof Element?event.target.closest('#balikanTable tr[data-row-number]'):null;
    const checkbox=row?.querySelector('.balikan-check');
...
let balikanShortcutsActive=false;
function syncBalikanShortcutListeners(active){
  if(active===balikanShortcutsActive)return;
  const input=document.querySelector('#balikanSearchInput')||balikanSearchInput;
  if(active){document.addEventListener('keydown',handleBalikanShortcutKeydown);input?.addEventListener('keydown',handleBalikanSearchKeydown);}
  else{document.removeEventListener('keydown',handleBalikanShortcutKeydown);input?.removeEventListener('keydown',handleBalikanSearchKeydown);}
  balikanShortcutsActive=active;
}
