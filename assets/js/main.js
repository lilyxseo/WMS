...
function renderDocumentValue(value){return isSafeDocumentUrl(value)?`<a href='${esc(String(value).trim())}' target='_blank' rel='noopener noreferrer'>Lihat Dokumen</a>`:esc(value??"");}
function renderTable(rows,presentRow=null){if(!rows.length) return `<div class='empty-card'><strong>Data kosong</strong><div>Tidak ada baris untuk sumber ini.</div></div>`;const presented=presentRow?rows.map(presentRow):rows;const headers=Object.keys(presented[0]);let h=`<div class='table-wrap'><table><thead><tr>${headers.map(x=>`<th>${esc(String(x).toUpperCase())}</th>`).join("")}</tr></thead><tbody>`;presented.forEach(r=>h+=`<tr>${headers.map(k=>`<td>${k==="dokumen"?renderDocumentValue(r[k]):esc(r[k])}</td>`).join("")}</tr>`);h+=`</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${rows.length} dari ${rows.length} data</span></div>`;return h;}
...
const TRANSACTION_COLUMN_LABELS={tanggal:"TANGGAL",from:"FROM",to:"TO",sku:"SKU",namaBarang:"NAMA BARANG",qty:"QTY",status:"STATUS",pic:"PIC",keterangan:"KETERANGAN",no_iseller:"NO ISELLER",netsuite:"NETSUITE",keterangan_lainnya:"KETERANGAN LAINNYA",lokasi_surat_jalan:"LOKASI SURAT JALAN",stockout:"STOCKOUT",dokumen:"DOKUMEN"};
function getTransactionPresentationCells(row,sheet="Barang Keluar"){const columns=sheet==="Barang Masuk"?BARANG_MASUK_PRESENTATION_COLUMNS:TRANSACTION_PRESENTATION_COLUMNS;return Object.fromEntries(columns.map(key=>[key,row?.[key]??""]));}
...
function getMovementColumns(rows=[]){return rows[0]?Object.keys(rows[0]._rawCells||{}):[...TRANSACTION_PRESENTATION_COLUMNS];}
