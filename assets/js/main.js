...
async function prefetchTransactionPage(mode,{page,limit,query,sort,filters,summaryKey}){const st=TABLE_STATE[mode];if(page<1||page>Math.ceil(st.total/limit))return;const source=transactionSource(mode),key=transactionPageKey({source,page,limit,query,filters,sort});if(TRANSACTION_PAGE_CACHE.get(source,key))return;const params=new URLSearchParams({page:String(page),limit:String(limit),sort,includeSummary:'0'});if(query)params.set('q',query);const endpoint=mode==='in'?'/api/barang-masuk':'/api/barang-keluar';try{const {res,data}=await fetchJsonSafe(`${endpoint}?${params}`);if(res.ok&&data?.success)TRANSACTION_PAGE_CACHE.set(source,key,{rows:normalizeBackendRows(data),total:Number(data.total)||0,summary:TRANSACTION_PAGE_CACHE.getSummary(summaryKey)?.summary||null,fetchedAt:Date.now()});}catch(err){console.debug(`[${source}] adjacent prefetch skipped`,err?.message||err);}}
...
try{const {res,data}=await fetchJsonSafe(`${endpoint}?${params}`,{signal:controller.signal});if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal memuat data');if(requestId!==st.requestId||controller.signal.aborted)return;
