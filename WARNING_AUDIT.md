...
| Referensi NETSUITE Kosong | Medium | A BULKY row has NULL/blank `netsuite`; BULKY sync identifies NETSUITE as the reconciliation metadata source. This is separate from accuracy mismatch. |
...
| Tanggal Invalid | Medium | Barang Masuk/Keluar `tanggal` is blank, not `YYYY-MM-DD`/`M/D/YYYY`/`MM/DD/YYYY`, or is an impossible calendar date. |
| QTY Invalid | High | Transaction QTY is NULL/blank/non-numeric. Zero and negative numeric values are not flagged because the repository has no confirmed prohibition. |
...
One authenticated `/api/inventory-warnings` request loads each complete authoritative Supabase source once (in parallel and in 1,000-row server-side batches), builds all enabled categories in one pass, then applies backend search/filter/sort/pagination. No warning uses browser arrays, page caches, preloads, or a previously opened page. Search is case-insensitive, contains-based, multi-word AND matching. Sorting has a stable SKU/ID tie-breaker.

The endpoint returns unfiltered category/severity totals from the same computed warning collection as detail rows. Responses are `no-store`. Inventory-version changes revalidate the visible Warning page while preserving filter/page state and table scroll offsets. If any source is unavailable, the endpoint fails closed instead of treating unavailable data as evidence of absence and producing false positives.
