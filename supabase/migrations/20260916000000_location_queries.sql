...
language sql stable security definer set search_path = public as $$
  select upper(trim(ks.lokasi_bulky)), trim(ks.sku), coalesce(max(nullif(trim(ks.nama_barang), '')), '-'), sum(coalesce(ks.stok_akhir, 0))
  from public.inventory_kartu_stok as ks
  where nullif(trim(ks.sku), '') is not null and nullif(trim(ks.lokasi_bulky), '') is not null
  group by upper(trim(ks.lokasi_bulky)), trim(ks.sku)
  having sum(coalesce(ks.stok_akhir, 0)) > 0
$$;
...
-- Make the new RPC signatures visible to PostgREST immediately after migration.
